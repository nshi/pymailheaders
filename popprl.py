...
class pop:
	...
	def __disconnect(self):
		...
		try:
			...
			if response != '+OK':
				print >> stderr, 'popprl (__disconnect): ' + \
				      'Logout failed.'
	...
	def __connect(self):
		...
		try:
			if self.__ssl:
				self.__connection = poplib.POP3_SSL( \
					self.__server)
			else:
	...
	def get_mail(self):
		...
		try:
			...
			for i in message_range:
				...
				if response[0][0:3] != '+OK':
					...
					raise Exception('Fetching messages' + \
							' failed')
				def b(x):
				...
				if re.search('^From', filtered_header[0]) == \
				   None:
		...
		self.__message_dict = self.__message_dict.fromkeys(uid_list, \
								   None)
